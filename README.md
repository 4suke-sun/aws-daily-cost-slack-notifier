...
EventBridge Rule (UTC 0:00)
    ...
    Lambda (Node.js 22)
        ...
        ├──▶ Secrets Manager    (Slack Webhook URL の取得)
...
| EventBridge Rule | 毎日 UTC 0:00（JST 9:00） |
| Secrets Manager | `/daily-cost-notifier/slack-webhook-url` |
...
| 変数名 | 説明 | デフォルト |
|-------|------|----------|
| `SECRET_NAME` | Secrets Manager のシークレット名 | （必須） |
| `TOP_N` | 通知するサービスの上位件数 | `5` |
